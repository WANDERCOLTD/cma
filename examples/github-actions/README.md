...
The `cma` plugin handles the **developer-experience layer**: `/cma:merge` from inside Claude Code, lock state, gate output streaming. For *serialisation* of merges to `main` when more than one contributor (human or AI agent) is active, the right primitive is **GitHub Merge Queue** plus a GHA workflow that runs the canonical gate.
...
|-------|--------------|-------------|
| **`cma` plugin** | `/cma:merge`, file lock, rebase, stream gate output, ratchet relock | Claude Code session |
...
This blocks direct pushes — including from `cma`. The plugin will still rebase + run the local gate, but it'll submit to the merge queue instead of pushing direct.
...
## Recommended `.merge-agent.json` when paired with GHMQ
...
Notes:
- `branchProtection: "error"` makes `cma` refuse to start a session in a repo with no branch protection. Once you've enabled GHMQ, that's the right posture.
