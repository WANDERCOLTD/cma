...
@test "CMA_DISABLE=1 → exits 0 without touching git, prints kill-switch message" {
  write_config '{ "gate": { "command": "true" } }'
  run env CMA_DISABLE=1 bash "$REPO_ROOT/bin/merge.sh" "$TMP_REPO"
  [ "$status" -eq 0 ]
...
@test "config disabled:true → exits 0 without touching git" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"disabled via .merge-agent.json"* ]]
}

@test "env var precedence: CMA_DISABLE=1 wins even with disabled:false in config" {
  write_config '{ "disabled": false, "gate": { "command": "true" } }'
  run env CMA_DISABLE=1 bash "$REPO_ROOT/bin/merge.sh" "$TMP_REPO"
  [ "$status" -eq 0 ]
  [[ "$output" == *"CMA_DISABLE=1"* ]]
}
...
@test "no .merge-agent.json → exits 1 with setup help" {
  run bash "$REPO_ROOT/bin/merge.sh" "$TMP_REPO"
  [ "$status" -eq 1 ]
...
# real cma repo (eat-our-own-dogfood on v0.2.1).

@test "lock acquired then released on disabled-via-env path" {
  write_config '{ "gate": { "command": "true" } }'
  CMA_DISABLE=1 bash "$REPO_ROOT/bin/merge.sh" "$TMP_REPO" >/dev/null
  [ ! -d "$TMP_REPO/.merge-agent.lock" ]
}
