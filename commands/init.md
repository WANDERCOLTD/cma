---
name: init
description: Bootstrap 4wd gate + ratchet + (optional) GitHub Actions into the current project. Detects the stack (Node, Python, shell) and drops the right starter files.
---

# /4wd:init

Set up 4wd for a fresh project — detects the stack, copies the right templates, and tells you what to do next.
...
3. Emit three files (more with `--with-gha`):
   - `.4wd.json` with the stack's default `gate.command`.
   ...
   - `.github/workflows/4wd-gate.yml` — only with `--with-gha`. Suppressed when `origin` isn't GitHub.
4. Touch `.gitignore` to add `.4wd.lock/`.
...
| `--merge` | Deep-merge into existing `.4wd.json` (preserves your fields). |
| `--dry-run` | Show what would happen without writing anything. |
| `--with-gha` | Also emit `.github/workflows/4wd-gate.yml`. Off by default to avoid clobbering existing CI. |
...
- jq is required (already a 4wd dependency).
